    ...
    public function admin_dashboard(Request $request)
    {
        ...
        $data['total_customers'] = User::where('user_type', 'customer')
            ...
            ->where(function ($query) {
                $query->where('is_wholeseller', 0)
                      ->orWhereNull('is_wholeseller');
            })
            ->count();
        ...
        $data['top_customers'] = User::select('users.id', 'users.name', 'users.avatar_original', DB::raw('SUM(grand_total) as total'))
            ...
            ->where(function ($query) {
                $query->where('users.is_wholeseller', 0)
                      ->orWhereNull('users.is_wholeseller');
            })
            ->orderByDesc('total')
            ->limit(6)
            ->get();
        ...
        $data['total_wholesellers'] = User::where('user_type', 'customer')
            // ->whereNotNull('email_verified_at')
            ->where('is_wholeseller', 1)
            ->count();
        
        $data['top_wholesellers'] = User::select('users.id', 'users.name', 'users.avatar_original', DB::raw('SUM(grand_total) as total'))
            ->join('orders', 'orders.user_id', '=', 'users.id')
            ->groupBy('orders.user_id')
            ->where('users.user_type', 'customer')
            ->where('users.is_wholeseller', 1)
            ->orderByDesc('total')
            ->limit(6)
            ->get();
