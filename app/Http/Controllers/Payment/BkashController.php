...
class BkashController extends Controller
{
    ...
    {
        $request_data = array(
            'app_key' => config('payment.bkash.app_key', env('BKASH_CHECKOUT_APP_KEY')), 
            'app_secret' => config('payment.bkash.app_secret', env('BKASH_CHECKOUT_APP_SECRET'))
        );
        $request_data_json = json_encode($request_data);

        $header = array(
            'Content-Type:application/json',
            'username:' . config('payment.bkash.username', env('BKASH_CHECKOUT_USER_NAME')),
            'password:' . config('payment.bkash.password', env('BKASH_CHECKOUT_PASSWORD'))
        );
