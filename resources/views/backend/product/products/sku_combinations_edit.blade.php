<!-- backend/product/products/sku_combinations_edit.blade.php -->
            ...
            <tr class="variant">
                ...
                <td>
                    <div class="input-group">
                        <input type="number" lang="en" name="discount_{{ $str }}" value="@php
                            if($stock != null){
                                echo $stock->discount ?? 0;
                            }
                            else{
                                echo 0;
                            }
                           @endphp" min="0" step="0.01" class="form-control" required>
                        <select class="form-control" name="discount_type_{{ $str }}">
                            <option value="amount" @if($stock && $stock->discount_type == 'amount') selected @endif>{{translate('Flat')}}</option>
                            <option value="percent" @if($stock && $stock->discount_type == 'percent') selected @endif>{{translate('Percent')}}</option>
                        </select>
                    </div>
                </td>
                <td>
