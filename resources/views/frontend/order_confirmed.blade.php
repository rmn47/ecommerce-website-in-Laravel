...
@section('script')
    ...
    <script>
        (function() {
            // Flag to track if the page has been loaded once
            var isFirstLoad = localStorage.getItem('orderConfirmationLoaded') !== 'true';

            // Set the flag on first load
            if (isFirstLoad) {
                localStorage.setItem('orderConfirmationLoaded', 'true');
            } else {
                // If the page is reloaded (refreshed), redirect to homepage
                window.location.href = '{{ route("home") }}';
                return; // Exit the script to prevent further execution
            }

            // Prevent refresh via keyboard shortcuts (F5, Ctrl+R, Cmd+R)
            document.addEventListener('keydown', function(e) {
                if (e.key === 'F5' || (e.ctrlKey && e.key === 'r') || (e.metaKey && e.key === 'r')) {
                    e.preventDefault();
                    alert('Refreshing is not allowed on this page. You will be redirected to the homepage.');
                    window.location.href = '{{ route("home") }}';
                }
            });

            // Prevent refresh via browser's refresh button or right-click reload
            window.addEventListener('beforeunload', function(e) {
                // This will trigger a confirmation dialog in most browsers
                e.preventDefault();
                e.returnValue = 'Refreshing is not allowed on this page. You will be redirected to the homepage.';
                return 'Refreshing is not allowed on this page. You will be redirected to the homepage.';
            });
            ...
            // Clean up the localStorage flag when navigating away (not refreshing)
            window.addEventListener('unload', function() {
                // Only clear the flag if navigating away, not refreshing
                if (!document.hidden) { // document.hidden is true during refresh
                    localStorage.removeItem('orderConfirmationLoaded');
                }
            });
        })();
    </script>
